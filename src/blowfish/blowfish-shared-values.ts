...
export class BlowfishSharedVar {
  private static OPTIMIZED_DEFAULT_HEX =
    BLOWFISH_CONSTANTS.OPTIMIZED_DEFAULT_HEX;
  static parsedBoxes: { p: Uint32Array; s: Uint32Array[] } =
    BlowfishSharedVar.parseStaticBoxes();
  static pbox_init: Uint32Array = this.parseENVBoxes(
    process.env.PBOX_INIT,
    'pbox_init',
  );
  static sbox_init_1: Uint32Array = this.parseENVBoxes(
    process.env.SBOX_INIT_1,
    'sbox_init_1',
  );
  static sbox_init_2: Uint32Array = this.parseENVBoxes(
    process.env.SBOX_INIT_2,
    'sbox_init_2',
  );
  static sbox_init_3: Uint32Array = this.parseENVBoxes(
    process.env.SBOX_INIT_3,
    'sbox_init_3',
  );
  static sbox_init_4: Uint32Array = this.parseENVBoxes(
    process.env.SBOX_INIT_4,
    'sbox_init_4',
  );

  private static parseENVBoxes(
    envVariable?: string,
    boxName?:
      | 'pbox_init'
      | 'sbox_init_1'
    ...
    if (envVariable) {
      return new Uint32Array(
        envVariable.split(',').map((hex) => parseInt(hex, 16)),
      );
    }
  ...
  private static parseStaticBoxes(): { p: Uint32Array; s: Uint32Array[] } {
    const boxes: { p: Uint32Array; s: Uint32Array[] } = {
      ...
      s: [
        ...
        new Uint32Array(256),
        new Uint32Array(256),
      ], // Four arrays of 256 elements each
    };
