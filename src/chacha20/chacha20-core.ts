...
import { ChaCha20Error, ChaCha20EncodingUtils, ChaCha20ParsingUtils } from "./utils";
...
export class ChaCha20Core {
    ...
    constructor(key: Uint8Array, nonce: Uint8Array, counter = 0) {
        if (!(key instanceof Uint8Array) || key.length !== 32) {
            throw ChaCha20Error.CoreConstructorError('Key must be 32 bytes (256 bits).')

        }
        if (!(nonce instanceof Uint8Array) || nonce.length !== 12) {
            throw ChaCha20Error.CoreConstructorError('Nonce must be 12 bytes (96 bits).')
    ...
    private initializeState() {
        ...
        for (let i = 0; i < 8; i++) {
            this.stateMatrix[4 + i] = ChaCha20ParsingUtils.readUint32LE(this.key, i * 4);
        }
        ...
        for (let i = 0; i < 3; i++) {
            this.stateMatrix[13 + i] = ChaCha20ParsingUtils.readUint32LE(this.nonce, i * 4);
        }
    }
    /**
