...
import { TwoFishUtils } from "./two-fish-utils";
...
export class TwoFishCore {
    ...
    constructor(password: string) {
        ...
        const sessionKey = this.initSessionKey(password);
    ...
    private initSessionKey(passphrase: string): Uint8Array {
        ...
        for (let i = 0; i < passphrase.length; i++) {
            key[i] = TwoFishUtils.charToSigned8BitInt(passphrase[i]);
        }

        // Truncate if the key is too long
        const keyLength = key.length;
        if (keyLength > TwoFishSharedValues.maxPasswordLength) {
            key = key.subarray(0, 32); // Truncate to 32 bytes if too long
        } else {
            const mod = keyLength % 8;

            // Add padding if needed to make length multiple of 8
        ...
        const keyCopy = new Uint8Array(finalKey); // Clone before wiping

        finalKey.fill(0); // Secure memory wipe
    ...
    private makeSession(key: Uint8Array) {
        ...
        const tempSBox = new Uint32Array(sessionMemory, 0, 1024);
        const tempSKeys = new Uint32Array(sessionMemory, 4096, 40);
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const wordIndex = i * 2;
            tempSBox[boxIndex] = this.rsMDSEncode(kWords[wordIndex], kWords[wordIndex + 1]);
        }
        /**
            TODO
        ...
        for (let i = 0, j = 0; i < 256; i++, j += 2) {
            ...
            TwoFishUtils.getSubKeyWord({
                numKeyBlocks64,
                keyWord0, keyWord1, keyWord2, keyWord3, subByte0: i, subByte1: i, subByte2: i, subByte3: i, subKeyWord: this.SUB_KEY_WORD
            });
    ...
    protected generateSubKeys({ keyWords, numKeyBlocks64, tempSKeys }: { keyWords: number[]; numKeyBlocks64: number; tempSKeys: Uint32Array }) {
        ...
        for (let i = 0, q = 0, j = 0; i < this.SUB_KEY_CNT / 2; i++, j += 2) {
            ...
            TwoFishUtils.getSubKeyWord({
                numKeyBlocks64, keyWord0: k0, keyWord1: k2, keyWord2: k4, keyWord3: k6, subByte0, subByte1, subByte2, subByte3, subKeyWord: this.SUB_KEY_WORD
            })
            // is  this.SUB_KEY_WORD mutated ?? 
            // test if subkey is mutated 

            // XOR the left key
            combinedLeft = this.SUB_KEY_WORD[0] ^ this.SUB_KEY_WORD[1] ^ this.SUB_KEY_WORD[2] ^ this.SUB_KEY_WORD[3]
            // Increment for next round of substitution
            q += this.SK_STEP;

            // Generate the second subkey word using odd indexed key words (k1, k3, k5, k7)
            TwoFishUtils.getSubKeyWord({
                numKeyBlocks64, keyWord0: k1, keyWord1: k3, keyWord2: k5, keyWord3: k7, subByte0, subByte1, subByte2, subByte3, subKeyWord: this.SUB_KEY_WORD
            })
            // XOR the right key
            combinedRight = this.SUB_KEY_WORD[0] ^ this.SUB_KEY_WORD[1] ^ this.SUB_KEY_WORD[2] ^ this.SUB_KEY_WORD[3]
            q += this.SK_STEP;
            ...
            tempSKeys[j + 1] = (combinedLeft << this.SK_ROTL) | (combinedLeft >>> (32 - this.SK_ROTL));
        }
    }
    protected rsMDSEncode(keyWord0: number, keyWord1: number): number {
