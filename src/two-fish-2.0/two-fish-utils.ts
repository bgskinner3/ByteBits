...
export class TwoFishUtils {
    ...
    static getSubKeyWord({ numKeyBlocks64, keyWord0, keyWord1, keyWord2, keyWord3, subByte0, subByte1, subByte2, subByte3,subKeyWord }: TGetSubKeyWordType) {
        ...
        subKeyWord[0] = this.MDS0[this.P0[subByte0] ^ this.substitutionMixer(keyWord0, 'b0')];
        subKeyWord[1] = this.MDS1[this.P0[subByte1] ^ this.substitutionMixer(keyWord0, 'b1')];
        subKeyWord[2] = this.MDS2[this.P1[subByte2] ^ this.substitutionMixer(keyWord0, 'b2')];
        subKeyWord[3] = this.MDS3[this.P1[subByte3] ^ this.substitutionMixer(keyWord0, 'b3')];
    ...
    static truncateAndPadKey(key: Uint8Array): Uint8Array {
        ...
        const truncatedKey = key.length > TwoFishSharedValues.maxPasswordLength
            ? key.subarray(0, TwoFishSharedValues.maxPasswordLength)
            : key;
    ...
    static reedSolomonTransformStep(val: number): number {
        ...
        const gfMult2 = ((topByte << 1) ^ ((topByte & 0x80) !== 0 ? TwoFishSharedValues.reedSolGalField : 0)) & 0xff;

        // Multiply topByte by 3 in GF(256) = gfMult2 XOR (topByte >>> 1) with polynomial adjustments
        const gfMult3 = (topByte >>> 1) ^ ((topByte & 0x01) !== 0 ? TwoFishSharedValues.reedSolGalField >>> 1 : 0) ^ gfMult2;

        // Rotate val left by 8 bits and XOR with the GF multiples in the right positions
        return (val << 8) ^ (gfMult3 << 24) ^ (gfMult2 << 16) ^ (gfMult3 << 8) ^ topByte;
    }
    /**
