...
import { TwoFishUtils, TwoFishError } from './utils';
...
export class TwoFishEncryptDecrypt {
  ...
  protected encryptBlock({
    ...
    for (let round = 0; round < this.ROUNDS; round += 2) {
      ...
      state2 = TwoFishUtils.rotateRight(state2, 1);

      state3 = TwoFishUtils.rotateLeft(state3, 1);
      ...
      state0 = TwoFishUtils.rotateRight(state0, 1);

      state1 = TwoFishUtils.rotateLeft(state1, 1);
  ...
  protected decryptBlock({
    ...
    for (let round = this.ROUNDS - 1; round >= 0; round -= 2) {
      ...
      state1 = TwoFishUtils.rotateRight(
        state1 ^ (t0 + 2 * t1 + this.core.s_Keys[k--]),
        1,
      );

      state0 = TwoFishUtils.rotateLeft(state0, 1);
      state0 ^= t0 + t1 + this.core.s_Keys[k--];
      ...
      state3 = TwoFishUtils.rotateRight(
        state3 ^ (t0 + 2 * t1 + this.core.s_Keys[k--]),
        1,
      );

      state2 = TwoFishUtils.rotateLeft(state2, 1);
      state2 ^= t0 + t1 + this.core.s_Keys[k--];
    }
  ...
  public encrypt(buffer: Uint8Array): Uint8Array {
    ...
    const paddedBuffer = TwoFishUtils.pkcs7Pad(buffer);
  ...
  public decrypt(buffer: Uint8Array): Uint8Array {
    ...
    return TwoFishUtils.pkcs7UnPad(plain);
  }
}

export class TestableTwoFishEncryptor extends TwoFishEncryptDecrypt {
  public testEncryptBlock(props: TEncryptDecryptBlockProps) {
    return this.encryptBlock(props);
  }
  public testDecryptionBlock(props: TEncryptDecryptBlockProps) {
    return this.decryptBlock(props);
  }
}
